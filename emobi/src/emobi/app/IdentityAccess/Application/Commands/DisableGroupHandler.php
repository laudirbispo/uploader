...
use app\IdentityAccess\Domain\{
    Models\Group,
    Models\GroupId,
    Contracts\UserProjectionRepository
};
final class DisableGroupHandler 
{
    ...
    public function execute(DisableGroupCommand $Command) 
    {
        $GroupId = GroupId::FromString($Command->getGroupId());
        $GroupHistory = $this->userRepository->get($GroupId);
        $GroupModel = Group::reconstituteRecordedEvents($GroupHistory);
        $GroupModel->disableGroup($Command->executedBy());
    }
